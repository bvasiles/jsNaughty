define([
    ...
    var
        ...
        findMatches = function (databank, pattern) {
            if (databank.union === undefined) {
                if (pattern.subject.type !== undefined) {
                    ...
                    return Utils.without(Utils.map(databank.propertyIndex[pattern.property], function (triple) {
                        var bindings = pattern.exec(triple);
                        return bindings === null ? null : { bindings: bindings, triples: [triple] };
                    }), null);
                }
        ...
        mergeMatches = function (existingMs, newMs, optional) {
            var compatibleMs,
                matches = Utils.without(Utils.flatten(Utils.map(existingMs, function (existingM) {
                    ...
                    compatibleMs = Utils.without(compatibleMs, null);
                    if (compatibleMs.length > 0) {
                        return Utils.flatten(Utils.map(compatibleMs, function (compatibleM) {
                            return {
                                bindings: Utils.extend({}, existingM.bindings, compatibleM.bindings),
        ...
        registerQuery = function (databank, query) {
            var s, p, o;
            if (query.filterExp !== undefined && !Utils.isFunction(query.filterExp)) {
                if (databank.union === undefined) {
                    ...
                    Utils.each(databank.union, function (databank) {
        ...
        resetQuery = function (query) {
            query.length = 0;
            query.matches = [];
            Utils.each(query.children, function (child) {
                resetQuery(child);
            });
            Utils.each(query.partOf, function (union) {
                resetQuery(union);
            });
        },
        ...
        updateQuery = function (query, matches) {
            if (matches.length > 0) {
                Utils.each(query.children, function (child, i) {
                    leftActivate(child, matches);
                });
                Utils.each(query.partOf, function (union, i) {
                    updateQuery(union, matches);
                });
                Utils.each(matches, function (match, i) {
                    if(match) {
        ...
        removeFromQuery = function (query, triple) {
            query.alphaMemory.splice(Utils.indexOf(query.alphaMindexOfemory, triple), 1);
            resetQuery(query);
            leftActivate(query);
        },
    ...
    rdf.fn = rdf.prototype = {
        ...
        add: function (triple, options) {
            ...
            if (triple.rdfquery !== undefined) {
                ...
                if (typeof triple === 'string') {
                    options = Utils.extend({}, { base: this.base(), namespaces: this.prefix(), source: triple }, options);
        ...
        dump: function (options) {
            ...
            options = Utils.extend({ namespaces: this.databank.namespaces, base: this.databank.base }, options || {});
