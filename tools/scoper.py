...
def __scopepaths(nested, root_pth):
    '''
    ...
    for key, value in nested.iteritems():
        ...
        if isinstance(value, dict):
            root_pth.append(key)
            __scopepaths(value, root_pth)
        
        elif isinstance(value, list):
            root_pth.append(key)
            for idx, subdict in enumerate(value):
                root_pth.append(idx)
                __scopepaths(subdict, root_pth)
            root_pth.pop()
...
def __keypaths(nested):
    ...
    for key, value in nested.iteritems():
        if isinstance(value, dict):
            for subkey, subvalue in __keypaths(value):
                yield [key] + subkey, subvalue
        elif isinstance(value, list):
            for idx, subdict in enumerate(value):
                for subkey, subvalue in __keypaths(subdict):
                    yield [key, idx] + subkey, subvalue
        else:
            yield [key], value
            

def __split_ref_key(str_key):
    '''
...
def __join_ref_key(keys):
    return '$%s' % ''.join([__pad(k) for k in keys])
...
class ScopeAnalyst:
    ...
    def __init__(self, in_file_path, scoper_js_path=None):
        ...
        __scopepaths(self.ast, [])
        
        # Extract names and scopes
        for (pth, key) in __keypaths(self.ast):
    ...
    def __ref_or_not(self, d):
        if d.has_key('$ref'):
            return self.get_ref_key(__split_ref_key(d['$ref']))
        return d
    ...
    def __get_thedef_scope(self, d):
        ...
        if thedefref:
            return self.get_ref_key(__split_ref_key(thedefref)).get('scope', {}).get('pth', None)
        else:
