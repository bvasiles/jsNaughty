...
log_path = os.path.join(output_path,
               'log_' + os.path.basename(training_sample_path))
...
#         open(log_path, 'w') as g:

    reader = UnicodeReader(f)
#     writer = UnicodeWriter(g)

    pool = multiprocessing.Pool(processes=4)
    results = pool.imap_unordered(processFile, reader)#, chunksize=10)
    
    while True:
        
        try:
            result = results.next(timeout=1)           # raises `TimeoutError`
            print result[0]
